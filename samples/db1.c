...
int create_db_env(struct test_struct *test_struct,
                                uint32_t db_cache_size)
{
    ...
    return ret;
err:
    close_db_env(test_struct);
    return ret;
}


int close_db_env(struct test_struct *test_struct)
{
    int ret = 0;
    ham_status_t st;
...
int main()
{
    ...
    int res = 0;
    char tmpname[40];
    sprintf(tmpname,"/tmp/temp_nameXXXXXX");
	memset(&test_struct,0,sizeof(test_struct));
	test_struct.db_file=tmpname;
    unlink(test_struct.db_file);
	if (file_exists(test_struct.db_file)==0)
		fprintf(stderr,"Creating [%s]\n",test_struct.db_file);
	else{
		fprintf(stderr,"[%s] Already exists\n",test_struct.db_file);
		if (remove(test_struct.db_file)!=0){
			fprintf(stderr,"Cannot remove [%s] - Aborting\n",test_struct.db_file);
			assert(0);// File Exists ?
		}
	}
	start = times(&start_timer);
	create_db_env(&test_struct,384);
	create_db(&test_struct,1);
	add_record_test(&test_struct);
	// do_flush(&test_struct);//uncomment this ..
	invalid_find_record_test(&test_struct);
	close_db_env(&test_struct);
    end = times(&end_timer);
