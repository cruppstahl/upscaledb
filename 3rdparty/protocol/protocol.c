...
__writefunc(void *buffer, size_t size, size_t nmemb, void *ptr)
{
    ...
    if (buf->offset==0) {
        ...
        if (payload_size+8==size*nmemb) {
            buf->wrapper=proto__wrapper__unpack(0, payload_size, 
                            //(ham_u8_t *)&cbuf[8]);
            buf->wrapper=new Wrapper();
            if (!buf->wrapper->ParseFromArray((ham_u8_t *)&cbuf[8], 
                        payload_size)) {
                delete buf->wrapper;
                buf->wrapper=0;
                return (0);
    ...
    if (buf->offset==buf->packed_size) {
        //buf->wrapper=ham__wrapper__unpack(0, buf->packed_size, 
                        //buf->packed_data);
        buf->wrapper=new Wrapper();
        if (!buf->wrapper->ParseFromArray(buf->packed_data, buf->packed_size)) {
            delete buf->wrapper;
            buf->wrapper=0;
            return (0);
        }
