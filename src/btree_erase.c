...
/*
 * replace two keys in a page - page is already locked
 */
static ham_status_t
...
/*
 * remove an item from a page - page is already locked
 */
static ham_status_t
        ...
        erase_scratchpad_t *scratchpad)
{
    ...
    if (newme) {
        if (slot==-1)
            slot=0;
        page=db_lock_page(db, scratchpad->txn, page, PAGE_NPERS_LOCK_WRITE);
        ...
        erase_scratchpad_t *scratchpad)
{
    ...
    if ((!leftpage || fewleft) && (!rightpage || fewright)) {
        if (lanchor!=page_get_self(parent)) {
            page=db_lock_page(db, scratchpad->txn, 
                    page, PAGE_NPERS_LOCK_WRITE);
            if (!page)
                return (0);
            rightpage=db_lock_page(db, scratchpad->txn, 
                    rightpage, PAGE_NPERS_LOCK_WRITE);
            if (!rightpage)
                return (0);
            ...
            if (!page)
                return (0);
            leftpage=db_lock_page(db, scratchpad->txn, 
                    leftpage, PAGE_NPERS_LOCK_WRITE);
            if (!leftpage)
                return (0);
        ...
        erase_scratchpad_t *scratchpad)
{
    ...
    if (btree_node_get_left(node)==page_get_self(sibpage)) {
        if (btree_node_get_left(sibnode)) {
            ...
            p=db_lock_page(db, scratchpad->txn, p, PAGE_NPERS_LOCK_WRITE);
        ...
        erase_scratchpad_t *scratchpad)
{
    ...
    ancpage=db_fetch_page(db, scratchpad->txn, anchor, 0);
    ancpage=db_lock_page(db, scratchpad->txn, ancpage, PAGE_NPERS_LOCK_WRITE);
    ancnode=ham_page_get_btree_node(ancpage);
