...
struct BlockCodecBase
{
  ...
  static bool insert(Index *index, const uint32_t *block_data,
...
class BlockKeyList : public BaseKeyList
{
  public:
    ...
    void grow_block(Index *index, int additional_size) {
      ...
      index->set_block_size(index->block_size() + additional_size);
  ...
  protected:
    ...
    virtual PBtreeNode::InsertResult insert_impl(size_t node_count,
      ...
      if (index->key_count() + 1 >= Index::kMaxKeysPerBlock
            || (index->used_size() + Index::kMaxSizePerInt
                        >= index->block_size() 
                && index->block_size() + Index::kMaxSizePerInt
                            >= Index::kMaxBlockSize))
        requires_split = true;

      // grow the block if it's size is 0 or if it overflows
      if (requires_split == false) {
        if (index->key_count() == 1
              && index->block_size() < Index::kInitialBlockSize)
          grow_block(index, Index::kInitialBlockSize - index->block_size());
        else if (index->used_size() + Index::kMaxSizePerInt
                        >= index->block_size())
          grow_block(index, Index::kMaxSizePerInt);
      }
    ...
    virtual void vacuumize_weak() {
      ...
      for (int i = 0; i < block_count; i++) {
        ...
        if (index->used_size() == 0)
          index->set_block_size(Index::kInitialBlockSize);
        else
