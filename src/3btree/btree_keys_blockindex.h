...
class BlockIndexKeyList : public BlockKeyList<BlockIndexIndex>
{
  public:
    enum {
      // Maximum keys per block
      kMaxKeysPerBlock = 128 // TODO run tests
    };

    // Constructor
    BlockIndexKeyList(LocalDatabase *db)
      : BlockKeyList<BlockIndexIndex>(db) {
    }
    ...
    int find(Context *, size_t node_count, const ham_key_t *hkey,
      ...
      uint32_t *data = uncompress_block(index);
    ...
    void copy_to(int sstart, size_t node_count, BlockIndexKeyList &dest,
      ...
      if (src_position_in_block > 0 || dst_position_in_block > 0) {
        ...
        uint32_t *ddata = dest.uncompress_block(dsti);
    ...
    void scan(Context *, ScanVisitor *visitor, uint32_t start, size_t count) {
      ...
      for (; it < end; it++) {
        uint32_t *data = uncompress_block(it);
    ...
    bool check_integrity(Context *, size_t node_count) const {
      if (!BlockKeyList<BlockIndexIndex>::check_integrity(node_count))
        return (false);
  ...
  private:
    ...
    uint32_t *uncompress_block(Index *index) const {
      return ((uint32_t *)get_block_data(index));
    }
    ...
    PBtreeNode::InsertResult insert_impl(size_t node_count, uint32_t key,
      ...
      if (index->key_count == 1) {
        ...
        uint32_t required_size = BlockIndexIndex::kInitialBlockSize;
      ...
      if (index->key_count + 1 >= kMaxKeysPerBlock) {
        ...
        if (key < index->value) {
          Index *new_index = add_block(block + 1,
                          BlockIndexIndex::kInitialBlockSize);
    ...
    // allocated
    void copy_blocks(Index *src, BlockIndexKeyList &dest, Index *dst) {
      dst->value     = src->value;
    ...
    void vacuumize_impl(bool internal) {
      ...
      for (int i = 0; i < block_count; i++) {
        ...
        if (index->used_size == 0)
          index->block_size = BlockIndexIndex::kInitialBlockSize;
        else
