 ...
 *
 * If the key is too big (exceeds |extkey_threshold_|) then it's offloaded
...
//
struct VariableLengthKeyList : public BaseKeyList
{
  ...
  VariableLengthKeyList(LocalDb *db)
    ...
    if (Globals::ms_extended_threshold)
      extkey_threshold_ = Globals::ms_extended_threshold;
    else {
      if (page_size == 1024)
        extkey_threshold_ = 64;
      else if (page_size <= 1024 * 8)
        extkey_threshold_ = 128;
      else {
        // UpfrontIndex's chunk size has 8 bit (max 255), and reserve
        // a few bytes for metadata (flags)
        extkey_threshold_ = 250;
      }
    }
  }
  ...
  void create(uint8_t *ptr, size_t range_size) {
    ...
    index_.create(data_, range_size, range_size / full_key_size());
  }
  ...
  size_t full_key_size(const ups_key_t *key = 0) const {
    if (!key)
      return 24 + index_.full_index_size() + 1;
    // always make sure to have enough space for an extkey id
    if (key->size < 8 || key->size > extkey_threshold_)
      return sizeof(uint64_t) + index_.full_index_size() + 1;
    return key->size + index_.full_index_size() + 1;
  }
  ...
  void key(Context *context, int slot, ByteArray *arena, ups_key_t *dest,
                  bool deep_copy = true) {
    ups_key_t tmp = {0};
    uint32_t offset = index_.get_chunk_offset(slot);
    uint8_t *p = index_.get_chunk_data_by_offset(offset);
  ...
  PBtreeNode::InsertResult insert(Context *context, size_t node_count,
    ...
    ups_key_t helper = {0};
    if (compressor_ && compress(key, &helper)) {
      key_flags = BtreeKey::kCompressed;
      key = &helper;
    }

    // When inserting the data: always add 1 byte for key flags
    if (key->size <= extkey_threshold_
          && index_.can_allocate_space(node_count, key->size + 1)) {
      uint32_t offset = index_.allocate_space(node_count, slot,
                      key->size + 1);
      uint8_t *p = index_.get_chunk_data_by_offset(offset);
      *p = key_flags;
  ...
  bool requires_split(size_t node_count, const ups_key_t *key) {
    ...
    if (key) {
      ...
      if (key->size > extkey_threshold_ || key->size < 8 + 1)
        required = 8 + 1;
    }
    else
      required = extkey_threshold_ + 1;
  ...
  void copy_to(int sstart, size_t node_count,
    ...
    for (size_t i = 0; i < to_copy; i++) {
      ...
      uint8_t *p = index_.get_chunk_data_by_offset(
                      index_.get_chunk_offset(sstart + i));
      uint8_t flags = *p;
      uint8_t *data = p + 1;

      dest.index_.insert(other_node_count + i, dstart + i);
      // Add 1 byte for key flags
      uint32_t offset = dest.index_.allocate_space(other_node_count + i + 1,
                      dstart + i, size + 1);
      p = dest.index_.get_chunk_data_by_offset(offset);
      *p = flags; // sets flags
  ...
  void check_integrity(Context *context, size_t node_count) const {
    ...
    for (size_t i = 0; i < node_count; i++) {
      if (key_size(i) > extkey_threshold_
      ...
      if (isset(get_key_flags(i), BtreeKey::kExtendedKey)) {
        ...
        blob_manager_->read(context, blobid, &record, 0, &arena);

        // compare it to the cached key (if there is one)
        if (extkey_cache_) {
          ExtKeyCache::iterator it = extkey_cache_->find(blobid);
          if (it != extkey_cache_->end()) {
  ...
  void change_range_size(size_t node_count, uint8_t *new_data_ptr,
    ...
    if (capacity_hint == 0) {
      capacity_hint = (new_range_size - index_.next_offset(node_count)
              - full_key_size()) / index_.full_index_size();
    ...
    // the capacity
    if (index_.next_offset(node_count) + full_key_size(0)
  ...
  void fill_metrics(btree_metrics_t *metrics, size_t node_count) {
    ...
    BtreeStatistics::update_min_max_avg(&metrics->keylist_index,
            (uint32_t)(index_.capacity()
                  * index_.full_index_size()));
    BtreeStatistics::update_min_max_avg(&metrics->keylist_unused,
            range_size_ - (uint32_t)index_.required_range_size(node_count));
  }
  ...
  void set_key_size(int slot, size_t size) {
    assert(size + 1 <= index_.get_chunk_size(slot));
    index_.set_chunk_size(slot, size + 1);
  }
  ...
  void erase_extended_key(Context *context, uint64_t blobid) {
    blob_manager_->erase(context, blobid);
    if (extkey_cache_) {
      ExtKeyCache::iterator it = extkey_cache_->find(blobid);
      if (it != extkey_cache_->end())
        extkey_cache_->erase(it);
    }
  }
  ...
  void get_extended_key(Context *context, uint64_t blob_id, ups_key_t *key) {
    if (!extkey_cache_)
      extkey_cache_.reset(new ExtKeyCache());
    else {
      ExtKeyCache::iterator it = extkey_cache_->find(blob_id);
      if (it != extkey_cache_->end()) {
    ...
    ByteArray arena;
    ups_record_t record = {0};
    blob_manager_->read(context, blob_id, &record, UPS_FORCE_DEEP_COPY,
                    &arena);
    (*extkey_cache_)[blob_id] = arena;
    arena.disown();
  ...
  uint64_t add_extended_key(Context *context, const ups_key_t *key) {
    ...
    uint64_t blob_id = blob_manager_->allocate(context, &rec,
                                      compressor_
                                          ? BlobManager::kDisableCompression
                                          : 0);
    assert(blob_id != 0);
    assert(extkey_cache_->find(blob_id) == extkey_cache_->end());
  ...
  bool compress(const ups_key_t *src, ups_key_t *dest) {
    assert(compressor_ != 0);

    // reserve 2 bytes for the uncompressed key length
    compressor_->reserve(sizeof(uint16_t));

    // perform compression, but abort if the compressed data exceeds
    // the uncompressed data
    uint32_t clen = compressor_->compress((uint8_t *)src->data, src->size);
    if (clen >= src->size)
      return false;

    // fill in the length
    uint8_t *ptr = compressor_->arena.data();
  ...
  void uncompress(const ups_key_t *src, ups_key_t *dest) {
    ...
    compressor_->decompress(ptr + sizeof(uint16_t),
                    src->size - sizeof(uint16_t), uclen);

    dest->size = uclen;
    dest->data = compressor_->arena.data();
  }
