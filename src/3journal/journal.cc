    ...
    m_count_bytes_after_compression(0)
{
  ...
  int algo = m_env->get_config().journal_compressor;
  if (algo)
                ...
                uint64_t lsn)
{
  ...
  // patched in later.
  uint32_t entry_position = m_buffer[idx].get_size();
  ...
  if (m_compressor.get()) {
    ...
    uint32_t len = m_compressor->compress((uint8_t *)key->data, key->size);
    if (len < key->size) {
      key_size = len;
  ...
  if (m_compressor.get()) {
    ...
    uint32_t len = m_compressor->compress((uint8_t *)record->data,
                        record_size);
    if (len < record_size) {
      record_size = len;
  ...
  m_buffer[idx].overwrite(entry_position,
                  &entry, sizeof(entry));
  m_buffer[idx].overwrite(entry_position + sizeof(entry),
                  &insert, sizeof(PJournalEntryInsert) - 1);
