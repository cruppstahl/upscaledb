        ...
        ham_record_t *record, ham_u32_t flags)
{
    ...
    if (st==SPLIT) {
         ...
         */
        newroot=db_alloc_page(db, txn, DB_READ_WRITE); 
        ...
        st=my_insert_nosplit(newroot, scratchpad.txn, &scratchpad.key, 
                scratchpad.rid, scratchpad.record, NOFLUSH|NOLOCK);
        if (st) {
        ...
        ham_offset_t rid, ham_record_t *record, ham_u32_t flags)
{
    ...
    if (!(flags&NOLOCK))
        page=db_lock_page(db, txn, page, DB_READ_WRITE);
        ...
        insert_scratchpad_t *scratchpad)
{
    ...
    /*
     * allocate a new page
     * TODO ist newpage schon gelockt für read/write??
     * eher nicht - könnte ein neues flag sein...
     */
    newpage=db_alloc_page(db, scratchpad->txn, DB_READ_WRITE); 
    ...
    if (btree_node_is_leaf(obtp)) {
        /* request write access */
        page=db_lock_page(db, scratchpad->txn, page, DB_READ_WRITE);
    ...
    if (cmp<=0)
        st=my_insert_nosplit(newpage, scratchpad->txn, key, rid, 
                scratchpad->record, flags|NOFLUSH|NOLOCK);
    else
