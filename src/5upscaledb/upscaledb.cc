                ...
                ups_record_t *record, uint32_t flags)
{
  ...
  if (!key) {
    ...
    return (db->set_error(UPS_INV_PARAMETER));
  }
  if (!record) {
                ...
                ups_record_t *record, uint32_t flags)
{
  ...
  if (db->get_flags() & UPS_READ_ONLY) {
    ...
    return (db->set_error(UPS_WRITE_PROTECTED));
  }
                ...
                uint64_t *keycount)
{
  ...
  return (db->set_error(db->count(txn, (flags & UPS_SKIP_DUPLICATES) != 0,
                  keycount)));
}
