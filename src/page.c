...
my_validage_page(ham_page_t *p)
{
    ...
    /*
     * not allowed: refcount > 1
     */
    ham_assert(page_ref_get(p)<=1, "invalid refcount %d in page 0x%llx",
            page_ref_get(p), page_get_self(p));
    ...
    /*
     * not allowed: shadowpage and in unref-list
     */
