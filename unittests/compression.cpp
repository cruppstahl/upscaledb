...
complex_journal_test(int library)
{
  ham_parameter_t params[] = {
    {HAM_PARAM_JOURNAL_COMPRESSION, (uint64_t)library},
    {0, 0}
  };
  ham_db_t *db;
  ham_env_t *env;
...
simple_record_test(int library)
{
  ...
  ham_db_t *m_db;
  ham_env_t *m_env;
  REQUIRE(0 == ham_env_create(&m_env, Utils::opath("test.db"), 0, 0, 0));
...
TEST_CASE("Compression/negativeOpenTest", "")
{
  ...
  ham_env_t *env;
  REQUIRE(HAM_INV_PARAMETER == ham_env_open(&env, Utils::opath("test.db"),
