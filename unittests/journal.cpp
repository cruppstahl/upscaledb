...
struct JournalFixture {
  ...
  void iterateOverEmptyLogTest() {
    ...
    PJournalEntry entry;
    ham_u8_t *data;
    REQUIRE(0 == j->get_entry(&iter, &entry, (void **)&data));
  ...
  void compareJournal(Journal *journal, LogEntry *vec, unsigned size) {
    ...
    do {
      REQUIRE(0 == journal->get_entry(&it, &entry, &aux));
      if (entry.lsn == 0)
        break;
  ...
  void recoverVerifyTxnIdsTest() {
    ...
    /* verify the lsn */
    Journal *j = m_environ->get_journal();
    REQUIRE(11ull == j->get_lsn());
    REQUIRE(5ull == m_environ->get_txn_id());
