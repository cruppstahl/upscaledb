...
struct HamsterdbFixture {
  ...
  void versionTest() {
    ...
    REQUIRE((uint32_t)HAM_VERSION_MAJ == major);
    REQUIRE((uint32_t)HAM_VERSION_MIN == minor);
    REQUIRE((uint32_t)HAM_VERSION_REV == revision);
  };
...
#undef PREP
  ...
  void overwriteLogDirectoryTest() {
    ham_env_t *env;
    ham_parameter_t ps[] = {
        { HAM_PARAM_LOG_DIRECTORY, (uint64_t)"data" },
        { 0, 0 }
    };
  ...
  void persistentFlagsTest() {
    ...
    uint32_t flags = HAM_ENABLE_DUPLICATE_KEYS;
    ...
    REQUIRE((uint64_t)HAM_TYPE_CUSTOM == query[0].value);
  ...
  void fileSizeLimitInMemoryTest() {
    ...
    ham_env_t *env;
    ham_parameter_t params[] = {
        {HAM_PARAM_FILE_SIZE_LIMIT, 3 * 16 * 1024},
        {0, 0}
    };

    REQUIRE(0 == ham_env_create(&env, 0, HAM_IN_MEMORY, 0, &params[0]));
    REQUIRE(0 == ham_env_create_db(env, &db, 1, 0, 0));
  ...
  void fileSizeLimitSplitTest() {
    ...
    ham_env_t *env;
    ham_parameter_t params[] = {
        {HAM_PARAM_FILE_SIZE_LIMIT, 3 * 16 * 1024}, // 3 pages
        {0, 0}
    };
  ...
  void fileSizeLimitBlobTest(bool inmemory) {
    ...
    ham_env_t *env;
    ham_parameter_t params[] = {
        {HAM_PARAM_FILE_SIZE_LIMIT, 2 * 16 * 1024}, // 2 pages
        {0, 0}
    };

    REQUIRE(0 == ham_env_create(&env, Utils::opath("test.db"),
                        inmemory ? HAM_IN_MEMORY : 0, 0, &params[0]));
